...
def main() -> int:
    ...
    # One paste instead of three. Everything the database needs to catch
    # up with this build, in the order it must run, in a single file —
    # because "run these three files in this order" is exactly the kind
    # of instruction that does not survive a busy day at a trading desk.
    sup = ROOT / "supabase"
    parts = []
    for name in ("01_schema.sql", "02_security.sql", "06_users.sql"):
        parts.append(f"-- ========== {name} ==========\n" + (sup / name).read_text(encoding="utf-8"))
    update = (
        "-- ============================================================\n"
    ...
    (sup / "UPDATE.sql").write_text(update, encoding="utf-8")
    print(f"wrote supabase/UPDATE.sql  ({len(update):,} bytes)")
    return 0
