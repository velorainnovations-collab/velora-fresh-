...
const VFSync = (function () {
  ...
  async function send(table, rows, isDelete, keyCols) {
    ...
    /* copies, not the queue's own rows: the strip below deletes columns,
       and a queued row mutilated by one attempt would stay mutilated in
       the saved queue — mixed shapes for ever after */
    const unique = Array.from(byKey.values()).map(r => Object.assign({}, r));
    return postRows(table, unionFill(unique), keyCols, 0);
  }

  /* PostgREST refuses a bulk insert whose rows do not all carry the same
     keys — "All object keys must match". A phone's saved queue can hold
     rows recorded by different builds of this app, older ones without
     the columns newer ones carry, and one submit then failed for ever.
     Every key any row has, every row gets; a column a row never knew is
     null, which is what it meant. */
  function unionFill(rows) {
    const keys = {};
    rows.forEach(r => { Object.keys(r).forEach(k => { keys[k] = 1; }); });
    rows.forEach(r => {
      Object.keys(keys).forEach(k => { if (!(k in r)) r[k] = null; });
    });
    return rows;
  }
